...
When Peter Shor published \citetitle{Shor:1997} \cite{Shor:1997} in \citeyear{Shor:1997}, not only were no actual
...
Shor's paper went substantially further by taking inspiration from from Simon in 1994 [\ldots cite], who refined an
oracle problem and showed a quantum solution in polynomial time that requires exponential time on a classical computer.
Shor tackled two number theory problems that underpin the Diffie-Helman \cite{Diffie:1976}, RSA \cite{Rivest:1978},
and ElGamal \cite{ElGamal:1985} public key encryption and digital signature schemes; namely quantum algorithms for
finding discrete logarithms and factor integers.
...
Shor's and Grover's algorithms using IBM's QISKIT SDK.
